...
Function Write-Log {
    ...
    ElseIf (Test-Path -Path $log) {
        ...
        If ($header) {
            $spacer = "##############################"
            $tmpmessage = $spacer + " $(Get-Date -Format f) " + $spacerParam ($cmg)
...
Function Install-CcmClient {
    ...
    Process {
        ...
        Else {
            Start-Process $exe -ArgumentList "CCMHOSTNAME=$source SMSMP=$mp /NOCRLCHECK" -Wait
        }
...
Function Uninstall-CcmClient {
    ...
    $client = Get-Process -Name CcmExec -ErrorAction SilentlyContinue
    If ($client) { Stop-Process -Name $client.Name -Force -Confirm:$false }
...
If ($adConnectionAddress) {
    Write-Log -Message "Found connection to $($adConnectionAddress.ComputerName), $($adconnectionAddress.RemoteAddress.IPAddressToString)" -Component "Get-ADConnectionInformation" -Type 1
    Write-Log -Message "Checking $($adConnectionAddress.ComputerName) for SMS information" -Component "Get-SMSBoundaryInformation" -Type 1

    # AD is available so we'll try and find an MP from here
    $boundaryInfo = Get-SMSBoundaryInformation -IP $adConnectionAddress.SelectedSourceAddress.IPAddress
...
If ($assignedSite -ne $connectionInfo.SiteCode) {
    ...
    If ($changed -eq $false) {
        Write-Log -Message "Was not able to assign proper Site Code" -Component "AssignmentStatus" -Type 3
        Write-Log -Message "Exiting with Code: 1" -Component "Get-ADConnectionInformation" -Type 1
        Exit 1
    }
